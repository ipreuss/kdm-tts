...
|----|------|-------------|----------|--------|-------|
| 1 | Strain Milestones | Auto-spawn disorder, injury, and strange resource cards (like fighting arts) instead of requiring manual search | P2 | Done | Completed 2025-12-05; all acceptance tests passing (confirm + undo for all 3 milestone types) |
...
Items referenced in expansion code but not found in `template_workshop.json`. These need either:
- Typo fixes in expansion code
- Addition of items to the TTS mod template
- Removal of incorrect entries from expansion code
...
- **CE variants**: Community Edition items have `(CE)` suffix. Non-CE variants should be used if CE expansion is disabled.
