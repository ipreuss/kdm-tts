...
Test.test("ACCEPTANCE: confirming milestone with disorder spawns disorder card", function(t)
    local world = TestWorld.create()
    
    -- Create a test milestone with disorder consequence
    local testMilestone = {
        title = "Test Disorder Milestone",
        consequences = { disorder = "Weak Spot" },
    }
    
    -- Manually inject and confirm
    world._strainModule.Test.ExecuteConsequences(testMilestone)
    
    -- Disorder should be spawned via Archive.Take
    t:assertTrue(world._archiveSpy:cardSpawned("Weak Spot", "Disorders"))
...
Test.test("ACCEPTANCE: confirming milestone with severe injury spawns injury card", function(t)
    local world = TestWorld.create()
...
Test.test("ACCEPTANCE: confirming milestone with strange resource spawns resource card", function(t)
    local world = TestWorld.create()
    
    local testMilestone = {
        title = "Test Strange Resource Milestone",
        consequences = { strangeResource = "Iron" },
    }
    
    world._strainModule.Test.ExecuteConsequences(testMilestone)
    
    -- Strange resource should be spawned via Archive.Take
    t:assertTrue(world._archiveSpy:cardSpawned("Iron", "Strange Resources"))
...
Test.test("ACCEPTANCE: unchecking milestone with spawned card logs manual removal message", function(t)
    local world = TestWorld.create()
    ...
    -- Execute spawns the card
    world._strainModule.Test.ExecuteConsequences(testMilestone)
    t:assertTrue(world._archiveSpy:cardSpawned("Weak Spot", "Disorders"))
    
    -- Reverse just logs a message (spawned cards can't be auto-removed)
    -- This doesn't throw an error - it just tells the player to manually remove
