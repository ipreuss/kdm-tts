...
---------------------------------------------------------------------------------------------------
-- Test: Real Showdown.Setup → ResourceRewards integration
---------------------------------------------------------------------------------------------------

Test.test("PROACTIVE: Showdown exports monster/level fields for cross-module access", function(t)
    -- This test catches the module export bug BEFORE it manifests at runtime.
    --
    -- Bug pattern:
    --   local Showdown = {}
    --   Showdown.monster = monster  -- assigns to internal table
    --   return { Setup = ..., Init = ... }  -- NEW table, no monster/level
    --
    -- Correct pattern:
    --   local Showdown = {}
    --   return Showdown  -- OR: return { monster = Showdown.monster, ... }

    -- Read the actual Showdown.ttslua source
    ...
    -- Check 1: Does Setup() assign to Showdown.monster and Showdown.level?
    local assignsMonster = content:match("Showdown%.monster%s*=%s*monster")
    local assignsLevel = content:match("Showdown%.level%s*=%s*level")
    t:assertNotNil(assignsMonster, "Setup should assign Showdown.monster")
    t:assertNotNil(assignsLevel, "Setup should assign Showdown.level")

    -- Check 2: Does the module return the internal Showdown table?
    -- Pattern: "return Showdown" at end of file (returns internal table directly)
    local returnsInternalTable = content:match("\nreturn%s+Showdown%s*$")

    -- THE KEY ASSERTION: Module must return internal table so assignments are visible
    t:assertNotNil(returnsInternalTable, "Module should return Showdown table directly (not a new table)")
end)
